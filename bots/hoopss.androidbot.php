...
$last_id = $last_checked_site_id['android_last_id'];

echo "SELECT id,link FROM android_google_site_links WHERE enabled='1' LIMIT $last_id, $limit;";
echo $site_que = $db->query("SELECT id,link FROM android_google_site_links WHERE enabled='1' LIMIT $last_id, $limit;");
while($site = $db->result($site_que))
{
	...
	if($curl->query($site['link']))
	{
       		...
       		$file_links = $get_links->getFilesAndroid($site['link'],$file_types_array,$keyword);
	}
	$db->query("UPDATE last_checked_site_id SET android_last_id='$last_id';");
	$last_id++;
}
